...
class PureHeelDetector:
    ...
    def analyze_frame(self, frame, timestamp, frame_number):
        ...
        for foot_name in ['left', 'right']:
            ...
            # In the analyze_frame function, add after detecting landmarks:
        if heel_data:
            for foot_name in ['left', 'right']:
                heel = heel_data[foot_name]
                if heel['usable']:
                    print(f"Frame {frame_number}: {foot_name} heel at y={heel['y']:.3f}, conf={heel['confidence']:.2f}")
            ...
            if self.detect_local_minimum(y_values, frame_number):
                ...
                if timestamp - last_step_time >= self.min_step_interval:
                    ...
                    print(f"HEEL STEP: {foot_name} at {min_point['timestamp']:.3f}s "
                          f"(y={min_point['y']:.3f}, conf={min_point['confidence']:.3f})")
...
def test_heel_detection(video_path, ground_truth_file):
    ...
    detector = PureHeelDetector(
        ...
        window_size=5,       # Look at 5-frame windows for local minima
...
if __name__ == "__main__":
    # Example usage
    video_file = "./data/test_videos/walk0.mp4"
    gt_file = "./data/test_videos/walk0_ground_truth.json"
    
    if video_file and gt_file:
        detected_steps, comparison = test_heel_detection(video_file, gt_file)
    else:
        print("Please provide both video and ground truth file paths.")